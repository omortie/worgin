...
def read_index():
    import json
    # load from file:
    with open('index_TEXT.json', 'r') as f:
        try:
            data = json.load(f)
...
def save_index(index):
    import json    
    # save to file:
    with open('index_TEXT.json', 'w') as f:
        json.dump(index, f)
    return 
...
size = len(index)
#index = crawl_web(index,"http://WWW.BBC.COM", 3)
index = crawl_domain(index,"https://en.wikipedia.org/wiki/Physics", 3)
