...
#    index = {}
    ...
    while tocrawl and depth <= max_depth: 
        page = tocrawl.pop()
        if page not in crawled and condition(depth,page): 
...
#    index = {} 
    ...
    while tocrawl and depth <= max_depth: 
        ...
        if page not in crawled and sub_domain in page and condition(depth,page): 
            whole_content, content = get_page2(page) 
            add_page_to_index(index, page, content) 
            union(next_depth, get_all_links(page,whole_content)) 
            crawled.append(page) 
...
def read_index():
    import json
    # load from file:
    with open('index_twitter.json', 'r') as f:
        try:
...
def save_index(index):
    import json    
    # save to file:
    with open('index_twitter.json', 'w') as f:
        json.dump(index, f)
    return 
...
def inline_lookup(index):
    ...
    print "See you soon :)."
    return 
def condition(depth,url):
    if depth>=1 and '/t.co/' not in url or "twitter.com" not in url:
        return False
    return True 
...
size = len(index)
index = crawl_web(index,twitter_hashtag(tag), 50)
#index = crawl_domain(index,twitter_hashtag(tag), 1)
